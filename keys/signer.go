...
// prefixLength is set to a value greater than 0, the signature will be capped to its first x bytes. This can be used to
// shorten the overall key length, but reduces the security of the signing. Note that the prefixLength set must be the
// same as set in HmacVerifier.
